import { IContactLeadSource, ILeadSource } from "../contacts/Interfaces"
...
export default function LeadSourceSelect({ leadSources, setLeadSources, className, addNewLeadSource, addExistingLeadSource, removeLeadSource }: ILeadSourceSelectProps) {
    ...
    useEffect(() => {
        const getLeadSources = async () => {
            ...
            try {
                const response = await axios.get(`/api/lead-sources/location/${process.env.NEXT_PUBLIC_LOCATION_ID}`,
                    {
                        headers: {
        ...
        <div>
            <div className={`flex flex-wrap ${className}`} onClick={() => { if (inputRef.current) inputRef.current.focus() }}>
                {leadSources.map((option: IContactLeadSource, index: number) => (
                    <Pill key={index} bgColor={"red-100"} textColor={"red-800"} showRemoveButton={true} onClick={() => removeLeadSourceFn(option)} value={option.content} />
                ))}
            ...
            </div>
            {
                (isTextBoxActive || isListBoxActive) ? (
                    <div className="cursor-default border border-gray-200 shadow-sm max-h-32 overflow-y-scroll" onMouseEnter={() => setIsListBoxActive(true)} onMouseLeave={() => setIsListBoxActive(false)}>
                        {textValue.length > 0 && !leadSources.some((leadSource) => leadSource.content === textValue.toLowerCase()) && !leadSourceList.some((leadSource) => leadSource.content === textValue.toLowerCase()) &&
                            <div className="cursor-default w-full hover:bg-gray-200 px-2 p-1" onClick={addNewLeadSourceFn}>
                                <span className="cursor-default">{textValue.toLowerCase()}</span>
                            </div>
                        }
                        {leadSourceList.map((option, index) => (
                            !leadSources.some((leadSource) => leadSource.content === option.content) && option.content.includes(textValue.toLowerCase()) &&
                            <div key={index} className="cursor-default w-full hover:bg-gray-200 px-2 p-1" onClick={() => addLeadSourceFn(option)}>
