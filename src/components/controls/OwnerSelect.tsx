...
function getOwners(): IOwner[] {
    ...
    const getOwnerList = async () => {
        try {
            const response = await axios.get(`/api/users/location/${process.env.NEXT_PUBLIC_LOCATION_ID}`);
...
export default function OwnerSelect({ owner, setOwner, className }: IOwnerSelectProps) {
    ...
    return (
        <div>
            <div className={`flex flex-wrap ${className}`} onClick={() => { if (inputRef.current) inputRef.current.focus() }}>
                <input ref={inputRef} onFocus={() => { setIsTextBoxActive(true); }} onBlur={() => {
                    setIsTextBoxActive(false)
                    if (!ownerList.some((option, index) => {
                ...
                }} className="outline-none flex-1" onChange={(e) => {
                    setTextValue(e.target.value);
                    if (e.target.value !== "")
                        ownerList.map((option, index) => {
                            if (option.fullName.toLowerCase() === e.target.value.toLowerCase()) {
                                addOwner(option);
                            }
                        })
                }} value={textValue} />
            </div>
            {
                ownerList.length > 0 && (isTextBoxActive || isListBoxActive) ? (
                    <div className="cursor-default border border-gray-200 shadow-sm overflow-scroll max-h-24" onMouseEnter={() => setIsListBoxActive(true)} onMouseLeave={() => setIsListBoxActive(false)}>
                        {ownerList.map((option, index) => (
                            option.fullName.toLowerCase().includes(textValue.toLowerCase()) &&
                            <div key={index} className="cursor-default w-full hover:bg-gray-200 px-2 p-1" onClick={() => addOwner(option)}>
