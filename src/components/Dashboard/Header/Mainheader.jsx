...
export default function Mainheader() {
  ...
  return (
    ...
    <Link href='#' className='flex items-center gap-1 text-white/80 text-xs'>
    ...
    <div className='flex items-center justify-end gap-3'>
        ...
        {<button onClick={_saveState} type='button' className='btn-gray bg-transparent border border-grey/50 px-4 text-sm py-2 hover:bg-grey hover:text-white hover:border-grey text-white'>Save as Draft</button>}
        {!save && <button onClick={_saveAndPublish} className='btn-gray bg-orange-600 border border-orange-600 hover:bg-transparent px-4 py-2 text-sm text-white'>Save & Publish</button>}
    </div>
</div>
  )
}
